...
void cd_add(toolbox_t *toolbox, char *dest, char *path)
{
    char **save;
    int count = 0;

    if (chdir(dest) == -1) {
        my_perror(path, errno);
    } else {
        dest = NULL;
        dest = getcwd(dest, 0);
...
void cd_back(toolbox_t *toolbox)
{
    ...
    recupoldpwd(toolbox);
    cd_add(toolbox, save, toolbox->agcv[1]);
}
...
{
    char *return_str = malloc(sizeof(char) * my_strlen(home) + my_strlen(path));
    int count = 0;
...
void cd_tilt(toolbox_t *toolbox)
{
    ...
    && find("HOME", toolbox->env[count]) == 0)
        count++;
    cd_add(toolbox, cd_tilt_2(toolbox->agcv[1], &toolbox->env[count][5])
    , cd_tilt_2(toolbox->agcv[1], &toolbox->env[count][5]));
}
...
void fc_cd(toolbox_t *toolbox)
{
    ...
    if (toolbox->agcv[1] != NULL && toolbox->agcv[1][0] == '-') {
        ...
        if (toolbox->agcv[1] == NULL || toolbox->agcv[1][0] == '~') {
            ...
            cd_add(toolbox, toolbox->agcv[1], toolbox->agcv[1]);
    }
}
